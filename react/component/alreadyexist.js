...
const FormComponent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateForm()) {
            try {
                if (formData.action === 'Amend') {
                    // Update the existing record
                    await axios.put(`http://localhost:8080/api/update/${formData.fromAccount}`, formData);
                    alert('Data updated successfully');
                } else {
                    // Create a new record
                    const response = await axios.post('http://localhost:8080/api/save', formData);
                    if (response.status === 400) {
                        alert(response.data); // Display the error message for existing record
                    } else {
